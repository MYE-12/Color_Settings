...
def get_frappe_version():
    return frappe.db.get_value("Installed Application",{"app_name":"frappe"},"app_version")
...
def boot_session(bootinfo):
	...
	if frappe.session['user']!='Guest':
		...
		bootinfo.color_settings = frappe.get_doc("Color Settings","Color Settings")

@frappe.whitelist()
def ignore_update_popup():
	if not frappe.db.get_single_value('Color Settings', 'disable_new_update_popup'):
